...
public class ProxyServiceAdmin {
    ...
    public String enableStatistics(String proxyName) throws ProxyAdminException {
        ...
        try {
            ProxyService proxy = ConfigHolder.getInstance()
            ...
            if (proxy != null) {
                ...
                log.error("Couldn't find the proxy service with name "
                        + proxyName + " to enalbe statistics");
                return FAILED;
            }
     ...
     * @param updateMode Whether the proxy service should be updated or added as a new service
    ...
    private void addProxyService(OMElement proxyServiceElement,
        ...
        try {
            if (proxyServiceElement.getQName().getLocalPart()
                ...
                if (ConfigHolder.getInstance().getSynapseConfiguration()
                        .getAxisConfiguration().getService(proxyName) != null) {
                    ...
                    try {
                        ...
                        if (updateMode) {
                            ...
                            if (fileName != null) {
                                ...
                                proxy.setFileName(MediationPersistenceManager.
                                        generateFileName(proxy.getName()));
                            }
                        }
                        ...
                        ConfigHolder.getInstance().getSynapseConfiguration().
                                removeProxyService(proxyName);
                        try{
                            if (ConfigHolder.getInstance().getAxisConfiguration()
                                    .getService(proxy.getName()) != null) {
                                ConfigHolder.getInstance().getAxisConfiguration()
                                        .removeService(proxy.getName());
                            }
    ...
    public String[] getAvailableTransports() throws ProxyAdminException {
        Object[] transports = ConfigHolder.getInstance().getAxisConfiguration()
                .getTransportsIn().keySet().toArray();
    ...
    public String[] getAvailableSequences() throws ProxyAdminException{
        try {
            Object[] sequences = ConfigHolder.getInstance().getSynapseConfiguration()
                    .getDefinedSequences().keySet().toArray();
    ...
    public String[] getAvailableEndpoints() throws ProxyAdminException {
        try {
            Object[] endpoints = ConfigHolder.getInstance().getSynapseConfiguration()
                    .getDefinedEndpoints().keySet().toArray();
    ...
    public String getEndpoint(String name) throws ProxyAdminException{
        String epXML = null;
        try {
            Endpoint ep = ConfigHolder.getInstance().getSynapseConfiguration()
                    .getDefinedEndpoints().get(name);
    ...
    public String startProxyService(String proxyName) throws ProxyAdminException {
        ...
        try {
            ProxyService proxy = ConfigHolder.getInstance().getSynapseConfiguration()
                    .getProxyService(proxyName);
            List pinnedServers = proxy.getPinnedServers();

            if (pinnedServers.isEmpty() || pinnedServers.contains(ServerManager.getInstance()
                    .getServerConfigurationInformation().getServerName())) {

                proxy.start(ConfigHolder.getInstance().getSynapseConfiguration());
            }
            if(log.isDebugEnabled()) {
    ...
    public String redeployProxyService(String proxyName) throws ProxyAdminException {
        try {
            ProxyService currentProxy = ConfigHolder.getInstance().getSynapseConfiguration()
                    .getProxyService(proxyName);
            if (currentProxy != null) {
                if(log.isDebugEnabled()) {
    ...
    private ProxyService proxyForName(String proxyName) throws ProxyAdminException {
        try {
            ProxyService ps = ConfigHolder.getInstance().getSynapseConfiguration()
                    .getProxyService(proxyName);
            if (ps != null) {
                return ps;
            } else {
