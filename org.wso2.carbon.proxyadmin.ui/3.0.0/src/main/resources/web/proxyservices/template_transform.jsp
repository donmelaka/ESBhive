...
<script type="text/javascript">
    function createProxy() {
        if (validateProxyName() && validateTargetEndpoint() &&
            validateWSDLOptions() && validateWSDLOptions() && validateXSLTKeys()) {
    ...
    if (submitted) {
        try {
            ...
            if (xsltKey == null || "".equals(xsltKey)) {
                throw new Exception("XSLT has not been specified for requests");
            }
            ...
            proxy.setInSeqXML("<inSequence xmlns=\"http://ws.apache.org/ns/synapse\"><xslt key=\"" + xsltKey +"\"/></inSequence>");
            if (responseXsltKey != null) {
                proxy.setOutSeqXML("<outSequence xmlns=\"http://ws.apache.org/ns/synapse\"><xslt key=\"" + responseXsltKey +"\"/><send/></outSequence>");
            } else {
